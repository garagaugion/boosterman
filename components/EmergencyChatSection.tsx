...
export default function EmergencyChatSection() {
  ...
  function sendManualAddress() {
    ...
    openWhatsApp(`${summary}\n\nIndirizzo inserito dal cliente:\n${address || 'Il cliente vuole scrivere via e civico su WhatsApp.'}`, address ? 'home_chat_manual_address' : 'home_chat_manual_address_empty');
  }

  function sendLocation() {
    trackBoostermanEvent('home_chat_sos_gps_click');
    if (!navigator.geolocation) {
      openWhatsApp(`${summary}\n\nHo premuto SOS GPS ma il telefono non supporta la posizione. Scrivo via e civico manualmente.`, 'home_chat_gps_unsupported');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        openWhatsApp(`${summary}\n\nHo premuto il pulsante SOS GPS.\nPosizione Google Maps:\nhttps://maps.google.com/?q=${latitude},${longitude}`, 'home_chat_gps_sent');
      },
      () => {
        setLocationLoading(false);
        openWhatsApp(`${summary}\n\nHo premuto SOS GPS ma non ho autorizzato la posizione. Scrivo via e civico manualmente.`, 'home_chat_gps_denied');
      },
    ...
    <section className="home-emergency-chat" aria-label="Assistente emergenza BOOSTERMAN">
      <div className="home-emergency-chat-head">
        ...
        <div>
          ...
          <p>Rispondi a poche domande. Se non sai cosa scegliere, puoi chiamare o inviare la posizione.</p>
        </div>
      </div>
        ...
        <div className="home-emergency-step">
          ...
          <div className="home-emergency-actions">
            <button type="button" className="home-sos-button" onClick={sendLocation} disabled={locationLoading}>
              🚨 {locationLoading ? 'Sto prendendo la posizione...' : 'SOS GPS'}
            </button>
